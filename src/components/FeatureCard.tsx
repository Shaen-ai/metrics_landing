...
export function FeatureCard({ icon: Icon, title, description, index }: FeatureCardProps) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={motionTransition(index * 0.08, 0.5)}
      className={cn(
    ...
    >
      <motion.div
        ...
        className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-muted text-muted-foreground transition-colors group-hover:bg-primary-muted group-hover:text-primary"
      >
        <Icon size={22} />
      </motion.div>
